...
viewGamesTab gi isAdmin saveDir mpn = do
   ...
   ok $ do
     ...
     table ! A.class_ "libraryButtons" $ do
       tr $ td ! A.class_ "buttonTD" $ H.a "My Rules "     ! A.class_ "button" ! href (toValue $ showRelURL $ Menu Lib gn)
       tr $ td ! A.class_ "buttonTD" $ H.a "My Modules "   ! A.class_ "button" ! href (toValue $ showRelURL $ Menu Modules gn)
     -- br >> b "Help files:" >> br
